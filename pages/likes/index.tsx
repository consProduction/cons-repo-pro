          ...
          <Grid item xs={12}>
            ...
            {!inmuebles && (
              <Typography variant="body1" color="text.secondary">
                No le has dado like a ningún inmueble
              </Typography>
            )}
          </Grid>
        </Grid>
  ...
  return (
    <Box sx={localStyles.card}>
      {favorite ? (
        <>
          <CustomImage
            src={`${process.ENV.IMG_URL}/newImg.php?nowatermark=1&url=${inmueble.url_inmueble}`}
            alt={inmueble.data.nombre}
            upperBoxStyles={{
              width: "100%",
              minHeight: 200,
              maxHeight: 200,
          ...
          <Box sx={localStyles.content}>
            ...
            <Typography variant="subtitle1">{`${
              inmueble.data.nombre.length > 29
                ? ucfirst(inmueble.data.nombre.toLocaleLowerCase()).substring(
                    0,
                    29
                  ) + "..."
                : ucfirst(inmueble.data.nombre.toLocaleLowerCase())
            }`}</Typography>
            <Typography
              variant="subtitle2"
              color="text.secondary"
              fontWeight="bold"
            >{`${inmueble.data.urbanizacion}, ${inmueble.data.municipio}, ${inmueble.data.Estado}`}</Typography>
            <Typography
              variant="subtitle2"
              sx={{ color: green[500] }}
              fontWeight="bold"
            >
              REF {inmueble.data.ref}
            </Typography>
          </Box>
        </>
      ) : (
      ...
      <Button
        sx={{ borderRadius: "0 0 20px 20px", textTransform: "none" }}
        variant="contained"
        disableElevation
        fullWidth
        onClick={() => redirect(inmueble.data.ficha_id)}
      >
        Ver inmueble
      </Button>
    </Box>

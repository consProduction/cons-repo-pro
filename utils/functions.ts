...
export const recortar = (dato: string) => {
  ...
  for (let i = 0; i < longitud && i < dato.length; i++) {
...
export const alertHandler = (
  status: number,
  mensajePersonalizado: boolean = false
) => {
  switch (status) {
    case 204:
      Swal.fire({
    ...
    case 401:
      ...
      Swal.fire({
        ...
        html: mensajePersonalizado
          ? mensajePersonalizado
          : "Autentiquese correctamente para poder ingresar data",
        icon: "error",
      });
      break;
    case 403:
    ...
    case 405:
      ...
      Swal.fire({
        ...
        html: mensajePersonalizado
          ? mensajePersonalizado
          : "Método de conexión no permitido",
        icon: "warning",
      });
      break;
    case 500:
...
export const createCookie = async (
  name: string,
  data: string,
  expire: string = "Thu, 01 Jan 2100 00:00:00 UTC",
  path: string = "/"
) => {
...
export const validarToken = async (
  ...
  const url = `${process.ENV.IMG_URL}/v2/auth/token_validation.php`;

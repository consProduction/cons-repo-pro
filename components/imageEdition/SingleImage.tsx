...
export const SingleImage: FC<SingleImageProps> = ({
  baseUrl,
  imagen,
  carpeta,
  id,
  ...
  const chooseFavoriteImage = async () => {
    const alerta = await Swal.fire({
      title: "¿Deseas establecer esta imagen como principal?",
    ...
    if (alerta.isConfirmed) {
      ...
      const url = `${process.ENV.BASE_URL}/v2/fotos/move.php?imagePath=${imagePath}&codigo=${id}`;
      try {
  ...
  const handleFile = (event: Event & { target: HTMLInputElement }) => {
    ...
    if (!event.target.files?.length || event.target.files?.length > 1) {
      ...
      event.target.files?.length &&
        Swal.fire({ text: `Por favor seleccione 1 imagen`, icon: "warning" });
  ...
  const chooseFile = () => {
    ...
    const inputFile: HTMLElement = document.querySelector(
      `#reemplazar`
    ) as HTMLElement;
    // Se añade el listener al input para almacenar las imagenes en el state
    inputFile.addEventListener("change", (e) =>
      handleFile(e as Event & { target: HTMLInputElement })
    );
  ...
  const reemplazarImagen = async (imagen: string) => {
    ...
    if (alerta.isConfirmed) {
      const url = `${process.ENV.BASE_URL}/v2/fotos/index.php`;
  ...
  const borrarImagen = async () => {
    ...
    if (alerta.isConfirmed) {
      const url = `${process.ENV.BASE_URL}/v2/fotos/index.php?name=${imagen}&location=${location}`;
      const options = {
        method: "DELETE",
        headers: {
          Accept: "application/json",
      ...
      <Box sx={styles.actions}>
        ...
        {imageSelected && (
          <Tooltip title="Guardar cambios">
            <IconButton
              onClick={() => reemplazarImagen(imagen)}
              size="small"
              color="success"
            >
              <SaveRoundedIcon />
        ...
        <Tooltip title={imagen}>
          <Typography variant="subtitle2">
            {imagen.length > 30 ? imagen.substr(0, 30) + "..." : imagen}
          </Typography>
        </Tooltip>
      </Box>
    </Box>

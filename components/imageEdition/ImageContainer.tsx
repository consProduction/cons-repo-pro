...
export const ImagesContainer: FC<PropsImages> = ({
  baseUrl,
  images,
  carpeta,
  id,
  ...
  const subirImagenes = async () => {
    const alerta = await Swal.fire({
      title: "¿Deseas subir las imagenes?",
    ...
    if (alerta.isConfirmed) {
      try {
        ...
        const url = `${process.ENV.BASE_URL}/v2/fotos/multiple.php`;
  ...
  return (
    <Box sx={styles.mainContainer}>
      <Box
        sx={{ display: "flex", alignItems: "center", flexFlow: "row nowrap" }}
      >
        <Typography
          fontWeight="bold"
          fontSize={20}
        ...
        {imagesToUpload && (
          <Tooltip title="Guardar imagenes">
      ...
      <Typography variant="subtitle2" sx={{ fontFamily: "Plus Jakarta Sans" }}>
        Imagenes de la carpeta {carpeta}
      </Typography>
